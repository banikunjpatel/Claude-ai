/** Add child — 3-step wizard: avatar → info → skills. */
...
export default function AddChildPage() {
  ...
  async function onSubmit(values: FormValues) {
    try {
      const newChild = await createChild.mutateAsync(values)
      navigate(`/app/children/success/${newChild.id}`)
    } catch (err) {
      setSubmitError((err as AppError).message ?? 'Failed to create profile.')
    }
  }
  ...
  return (
    <div className="bg-ck-neutral-50 min-h-screen pb-28">
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        {/* Heading */}
        <h1 className="text-2xl font-extrabold text-ck-neutral-900">Add a child</h1>
        ...
        {step === 1 && (
          <div>
            {/* Preview card (shown when name already filled from step 2) */}
            {displayName && (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        ...
                        style={{ color: isSelected ? skill.text : '#404040' }}
                      >
                        {skill.name}
                      </p>
                      <p
        ...
        <div className="max-w-lg mx-auto flex items-center gap-3">
          ...
          {step < 3 ? (
            ...
            <button
              ...
              disabled={skillFocuses.length === 0 || createChild.isPending}
              className={[
                'flex-1 bg-ck-primary-500 text-white rounded-full py-3 text-sm font-bold transition-all',
                skillFocuses.length === 0 || createChild.isPending
                  ? 'opacity-50 cursor-not-allowed'
                  : 'hover:bg-ck-primary-600',
              ].join(' ')}
            >
              {createChild.isPending ? 'Creating…' : 'Create profile →'}
            </button>
          )}
        </div>
      </div>
